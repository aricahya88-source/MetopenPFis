...
type Roster = {
  ...
  submission?:{submission_id:string; link_url:string; file_url:string; file_name?:string; content_html?:string; submitted_at?:string; version?:number; articles?:Array<{slot_no:number;url?:string;file_url?:string;file_name?:string}>}|null;
  post_count?:number;
...
type AssessmentResponse = EvalResponse & { row:Roster };
...
export default function Gradebook(){
  ...
  const[importRows,setImportRows]=useState<ImportRow[]>([]),[importName,setImportName]=useState(''),[busy,setBusy]=useState(false),[detailLoading,setDetailLoading]=useState('');
  ...
  const open=async(r:Roster)=>{setSelected(r);setScore(Number(r.grade?.score||0));setFeedback(r.grade?.feedback_html||'');setPublished(r.grade?.published!==false);setRubricScores({});setDetailLoading(r.user.user_id);setMsg('');try{const x=await api<AssessmentResponse>('adminStudentAssessment',{activity_id:activity,user_id:r.user.user_id});setSelected(x.row);setScore(Number(x.row.grade?.score||0));setFeedback(x.row.grade?.feedback_html||'');setPublished(x.row.grade?.published!==false);setRubricScores(x.evaluation?.scores||{})}catch(e){setMsg(e instanceof Error?e.message:String(e))}finally{setDetailLoading('')}};
  ...
  const save=async()=>{if(!selected)return;try{let savedGrade:Roster['grade']=null;if(localRubric){if(!rubricComplete){setMsg('Lengkapi skor 1–4 pada seluruh kriteria rubrik.');return;}const result=await api<{grade:Roster['grade'];total_score:number}>('adminSaveRubricEvaluation',{activity_id:activity,user_id:selected.user.user_id,submission_id:selected.submission?.submission_id||'',scores:rubricScores,feedback_html:feedback,published});savedGrade=result.grade;setMsg(`Rubrik tersimpan. Nilai akhir ${rubricTotal}/100.`)}else{if(score<0||score>100){setMsg('Nilai harus 0–100.');return;}savedGrade=await api<Roster['grade']>('adminSaveGrade',{activity_id:activity,user_id:selected.user.user_id,submission_id:selected.submission?.submission_id||'',score,feedback_html:feedback,published});setMsg('Nilai tersimpan.')}if(savedGrade){setSelected(v=>v?{...v,grade:savedGrade}:v);setRows(list=>list.map(x=>x.user.user_id===selected.user.user_id?{...x,grade:savedGrade}:x))}}catch(e){setMsg(e instanceof Error?e.message:String(e))}};
  const evidence=useMemo(()=>rows.filter(r=>current?.type==='discussion'?(Number(r.post_count)||0)>0:!!r.submission),[rows,current?.type]);
  const exportRows=async()=>{if(!current)return;const XLSX=await import('xlsx');const data=evidence.map(r=>({activity_id:activity,user_id:r.user.user_id,submission_id:r.submission?.submission_id||'',NIM:r.user.nim,Nama:r.user.name,Kelas:r.user.class_name||'',Aktivitas:current.title,Status:current.type==='discussion'?`${r.post_count||0} post`:(r.submission?'Sudah mengumpulkan':'Belum'),URL:r.submission?.link_url||'',File:r.submission?.file_url||'',Nilai:r.grade?.score??'',Feedback:strip(r.grade?.feedback_html||'')}));const ws=XLSX.utils.json_to_sheet(data);ws['!cols']=[{hidden:true},{hidden:true},{hidden:true},{wch:15},{wch:28},{wch:14},{wch:40},{wch:20},{wch:34},{wch:34},{wch:12},{wch:70}];const guide=XLSX.utils.aoa_to_sheet([['PETUNJUK'],['Isi kolom Nilai dan/atau Feedback. Isi jawaban lengkap tetap dibaca di LMS agar export tetap ringan.'],['Jangan mengubah activity_id, user_id, submission_id.'],['Untuk lima tugas utama, penilaian rubrik lebih disarankan langsung melalui Gradebook LMS.']]);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'PENILAIAN');XLSX.utils.book_append_sheet(wb,guide,'PETUNJUK');XLSX.writeFile(wb,`METOPEN_${activity}_${new Date().toISOString().slice(0,10)}.xlsx`)};
      ...
      <div className="stack">{selected&&<>
        {localRubric?<div className="lecturer-assessment-grid">
          <GlassCard className="assessment-evidence-panel"><div className="row between wrap gap"><div><span className="eyebrow">HASIL KERJA MAHASISWA</span><h3>{selected.user.name}</h3><p className="muted">{selected.user.nim}{selected.submission?.version?` • Submission v${selected.submission.version}`:''}{selected.submission?.submitted_at?` • ${new Date(selected.submission.submitted_at).toLocaleString('id-ID')}`:''}</p></div>{detailLoading===selected.user.user_id?<span className="badge">Memuat jawaban...</span>:structuredEvidence&&<span className="badge success"><CheckCircle2/>Form terstruktur</span>}</div>
            {detailLoading===selected.user.user_id?<p className="muted">Memuat isi tugas mahasiswa...</p>:selected.submission?<><div className="assessment-full-evidence"><RichHtml html={selected.submission.content_html||''}/></div>{activity==='TASK5_PROPOSAL'&&!selected.submission.file_url?<div className="error-box">Proposal PDF belum terlampir pada submission ini.</div>:null}{activity==='TASK1_ISSUE'&&selected.submission.articles?.length?<div className="stack small-gap"><strong>5 sumber artikel</strong>{selected.submission.articles.map(a=><div className="article-source-view" key={a.slot_no}><span className="badge">Artikel {a.slot_no}</span>{a.url&&<a target="_blank" rel="noreferrer" className="button soft compact" href={a.url}><ExternalLink/>Link</a>}{a.file_url&&<a target="_blank" rel="noreferrer" className="button soft compact" href={a.file_url}><ExternalLink/>{a.file_name||'PDF'}</a>}</div>)}</div>:null}<div className="row wrap gap">{selected.submission.link_url&&<a target="_blank" rel="noreferrer" className="button soft compact" href={selected.submission.link_url}><ExternalLink/>Buka URL</a>}{selected.submission.file_url&&<a target="_blank" rel="noreferrer" className="button soft compact" href={selected.submission.file_url}><ExternalLink/>Buka File</a>}</div></>:<p className="muted">Belum ada submission.</p>}
          </GlassCard>
